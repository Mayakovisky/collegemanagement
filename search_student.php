...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Query to search students by registration number (or adjust this to use name, email, etc.)
    $query = "SELECT * FROM students WHERE registration_number LIKE '%$search_query%'"; 
    $result = $conn->query($query);

    if ($result->num_rows > 0) {
        echo "<table border='1' cellspacing='0' cellpadding='10'>";
        echo "<tr style='background-color: #f2f2f2;'><th>ID</th><th>First Name</th><th>Last Name</th><th>Email</th><th>Program</th><th>Status</th><th>Action</th></tr>";
